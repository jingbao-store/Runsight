...
NEW_APK_NAME="runsight-v${VERSION_NAME}.apk"
...
if command -v shasum >/dev/null 2>&1; then
  APK_SHA256=$(shasum -a 256 "$NEW_APK_PATH" | awk '{print $1}')
elif command -v sha256sum >/dev/null 2>&1; then
  APK_SHA256=$(sha256sum "$NEW_APK_PATH" | awk '{print $1}')
else
  APK_SHA256="(sha256 工具不可用)"
fi
echo "🔒 SHA256: $APK_SHA256"

# 复制一份到项目根目录便于分发
ROOT_COPY="./$NEW_APK_NAME"
cp -f "$NEW_APK_PATH" "$ROOT_COPY"
echo "📤 已复制到项目根目录: $ROOT_COPY"
